...
function classifyError(error) {
    ...
    if (text.includes("no process attached") || text.includes("open_process") || text.includes("not attached"))
        return "NO_PROCESS";
    if (text.includes("econnrefused") || text.includes("timed out") || text.includes("bridge"))
        return "BRIDGE_UNAVAILABLE";
    if (text.includes("no scan results") || text.includes("no previous scan"))
        return "NO_SCAN";
    if (text.includes("invalid address") || text.includes("address"))
        return "INVALID_ADDRESS";
    if (text.includes("breakpoint") || text.includes("debug register"))
        return "BREAKPOINT_ERROR";
    if (text.includes("permission") || text.includes("denied") || text.includes("not writable"))
        return "PERMISSION_DENIED";
    if (text.includes("timeout"))
        return "TIMEOUT";
    if (text.includes("to undo") || text.includes("cannot undo"))
        return "UNDO_NOT_SUPPORTED";
    if (text.includes("no snapshot"))
        return "NO_SNAPSHOT";
    if (text.includes("invalid args") || text.includes("required"))
        return "INVALID_ARGS";
    return "UNKNOWN";
}
function withErrorClass(result, error) {
...
function pushUndo(item) { pushCapped(session.undoStack, item, MAX_UNDO); }
function pushEvidence(item) { pushCapped(session.evidence, item, MAX_EVIDENCE); }
function pushHypothesis(item) { pushCapped(session.hypotheses, item, MAX_HYPOTHESES); }
function pushAudit(item) { pushCapped(session.audit, item, MAX_AUDIT); }
function pushRecentEvent(text) {
    session.recentEvents.push({ text, ts: Date.now() });
    if (session.recentEvents.length > 5)
        session.recentEvents.shift();
}
function updateSession(toolName, args, result) {
    session.calls.push({
        tool: toolName,
        ts: Date.now(),
...
export function createToolDefs(client) {
    return [
        // ── 连接 / 状态 ────────────────────────────────────────────────
        {
            name: 'ce_status',
        ...
        },
        {
            ...
            async execute(args, client) {
                ...
                if (!address || !Number.isFinite(value))
        ...
        },
        {
            ...
            async execute() {
                ...
                return {
                    ...
                    recent_calls: session.calls.slice(-20).reverse().map((c) => ({ tool: c.tool, ts: c.ts, ok: c.ok, error_class: c.error_class || null })),
                };
            },
        },
        {
            ...
            async execute(args) {
                const entries = [];
                session.cache.forEach((v, k) => entries.push({ key: k, ...v }));
                const limit = Math.min(Number(args.limit) || 100, 1000);
        ...
        },
        {
            ...
            async execute() {
                if (!snapshot)
                    return { success: false, error: 'no snapshot saved', error_class: 'NO_SNAPSHOT' };
        ...
        },
        {
            ...
            async execute(args) {
                ...
                if (action === 'add') {
                    ...
                    if (!statement)
                        return { success: false, error: 'statement is required for add', error_class: 'INVALID_ARGS' };
                ...
                const entries = id ? session.hypotheses.filter((h) => h.id === id) : session.hypotheses;
        ...
        },
        {
            ...
            async execute(args, client) {
                ...
                if (!last)
                    return { success: false, error: 'no dangerous operation to undo', error_class: 'NOTHING_TO_UNDO' };
                if (last.kind === 'lock' && last.address) {
                    const addr = String(last.address);
                    const addrNum = Number.parseInt(addr.replace(/^0x/i, ''), 16);
                    if (!Number.isFinite(addrNum))
                        return { success: false, error: 'cannot undo lock: invalid address', error_class: 'INVALID_ADDRESS' };
                ...
                if (last.kind === 'write_memory' && last.address) {
                    if (!Array.isArray(last.before))
                        return { success: false, error: 'cannot undo write_memory: previous bytes unknown', error_class: 'UNDO_NOT_SUPPORTED' };
                ...
                if (last.kind === 'write_string' && last.address) {
                    if (last.before === null || last.before === undefined)
                        return { success: false, error: 'cannot undo write_string: previous string unknown', error_class: 'UNDO_NOT_SUPPORTED' };
        ...
        },
        {
            ...
            async execute(args) {
                ...
                if (engine)
                    pb.engine_note = engine === 'unity' ? 'Unity/IL2CPP 优先尝试 float/double，注意 UI 显示副本。' : engine === 'ue' ? 'UE 通常需要指针链，动态堆较多。' : engine === 'godot' ? 'Godot 脚本值可能在 VM 堆中。' : '';
        ...
        },
        {
            ...
            async execute(args) {
                ...
                if (action === 'add') {
                    ...
                    if (!claim)
                        return { success: false, error: 'claim is required for add', error_class: 'INVALID_ARGS' };
                    const entry = {
        ...
        },
        {
            ...
            async execute(args) {
                ...
                if (args.engine === 'unity')
                    mission.engine_note = 'Unity/IL2CPP 优先尝试 float/double，注意 UI 显示副本。';
        ...
        },
        {
            ...
            async execute() {
                ...
                if (session.locks.size > 0) {
                    lines.push('## Locked');
                    lines.push('');
                    for (const addr of session.locks)
                        lines.push(`- ${addr}`);
                    lines.push('');
                }
        ...
        },
        {
            ...
            async execute() {
                ...
                if (session.evidence.length > 0)
                    lines.push(`已记录 ${session.evidence.length} 条证据。`);
                if (session.hypotheses.length > 0)
                    lines.push(`已有 ${session.hypotheses.length} 个假设。`);
                if (session.audit.length > 0)
                    lines.push(`执行了 ${session.audit.length} 个危险操作。`);
        ...
        },
        {
            ...
            async execute(args, client) {
                ...
                if (mode === 'string') {
                    ...
                    if (res && res.success !== false)
                        pushUndo({ kind: 'write_string', address, before, after: text, wide, ts: Date.now() });
                    return res;
                }
                if (mode === 'many') {
                    let addresses = Array.isArray(args.addresses) ? args.addresses.map(String) : [];
...
function recordAutoEvidence(toolName, args, result) {
    if (!result || result.success === false)
        return;
    let entry = null;
    if (toolName === 'ce_scan') {
    ...
    if (entry) {
        ...
        entry.ts = Date.now();
        pushEvidence(entry);
        if (session.evidence.length > 200)
            session.evidence.shift();
    }
}
function buildTool(ctx, client, def) {
        ...
        return defineTool({
            ...
            async execute(args, exec) {
                ...
                const packs = Array.isArray(args.packs)
                    ? args.packs.filter((name) => typeof name === 'string' && name.length > 0)
                    : [];
                const lines = [];
                let schemas = [];
                try {
                ...
                if (unlockNames.length > 0) {
                    ...
                    const invalid = unlockNames.filter((name) => !allOwnSchemas.some((schema) => schema.name === name));
                    if (packs.length > 0) {
        ...
        async execute(args) {
            try {
                if (def.execute) {
                    ...
                    if (def.dangerous && res && res.success !== false)
                        pushAudit({ tool: def.name, args, ts: Date.now() });
                    return res;
                }
                ...
                if (def.dangerous && wrapped && wrapped.success !== false)
                    pushAudit({ tool: def.name, args, ts: Date.now() });
                return wrapped;
            }
            catch (err) {
 ...
 */
function unlockedFromEvents(session) {
    ...
    for (const event of session.events) {
        if (event.type !== 'tool/call')
            continue;
        if (event.data?.name !== 'ce_tool_search')
            continue;
        let args;
        try {
            ...
            if (Array.isArray(args.packs)) {
                for (const pack of args.packs) {
                    if (typeof pack !== 'string')
                        continue;
                    for (const name of TOOL_PACKS[pack] || [])
                        unlocked.add(name);
                }
            }
        }
...
function buildStats() {
    return {
        ...
        recent_calls: session.calls.slice(-10).reverse().map((c) => ({ tool: c.tool, ok: c.ok })),
        recent_events: session.recentEvents.slice(-5).reverse(),
    };
}
function renderStatusHtml() {
    const s = buildStats();
    const lockList = s.locked_addresses.map((a) => `<li><code>${a}</code></li>`).join('') || '<li>none</li>';
    return `<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8" />
...
<div class="card"><h2>Recent calls</h2><ul>${s.recent_calls.map((c) => `<li>${c.ok ? '✅' : '❌'} ${c.tool}</li>`).join('') || '<li>none</li>'}</ul></div>
...
function injectStatusPanel(html) {
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => {
        ...
        if (webServer && typeof webServer.register === 'function') {
            try {
                webDisposers.push(webServer.register({ kind: 'exact', path: '/ce-status', handler: async (_req, res) => { res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' }); res.end(renderStatusHtml()); } }));
                webDisposers.push(webServer.register({ kind: 'exact', path: '/ce-status/api', handler: async (_req, res) => { res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' }); res.end(JSON.stringify(buildStats())); } }));
            }
            catch (err) {
                try {
                    ctx.logger.warn(`dsh-cheatengine: failed to register /ce-status routes: ${String((err && err.message) || err)}`);
                }
                catch { /* ignore */ }
            }
        }
        ...
        const disposeFilter = ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
            ...
            try {
                if (!assembled || !Array.isArray(assembled.tools))
                    return assembled;
                const unlocked = unlockedFromEvents(context?.agent?.session);
                const keep = new Set([...RESIDENT_TOOLS, ...unlocked]);
                return {
                    ...assembled,
                    tools: assembled.tools.filter((tool) => !isOwnTool(tool.name) || keep.has(tool.name)),
                };
            }
            catch (err) {
                ...
                return {
                    ...assembled,
                    tools: assembled.tools.filter((tool) => !isOwnTool(tool.name) || RESIDENT_TOOLS.has(tool.name)),
                };
            }
        });
        return () => {
