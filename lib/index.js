...
import { createSessionState, updateSession, pushUndo, pushEvidence, pushHypothesis, pushAudit } from './session.js';
            ...
            try {
                if (def.execute) {
                    ...
                    if (def.dangerous && res && res.success !== false)
                        pushAudit(session, { tool: def.name, args, ts: Date.now() });
                    return res;
                }
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => {
        ...
        if (webServer && typeof webServer.register === 'function') {
            try {
                webDisposers.push(webServer.register({ kind: 'exact', path: '/ce-status', handler: async (_req, res) => { res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' }); res.end(renderStatusHtml(session)); } }));
                webDisposers.push(webServer.register({ kind: 'exact', path: '/ce-status/api', handler: async (_req, res) => { res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' }); res.end(JSON.stringify(buildStats(session))); } }));
            }
            catch (err) {
                try {
