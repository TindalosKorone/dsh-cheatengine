...
export function pushUndo(s, item) { pushCapped(s.undoStack, item, MAX_UNDO); }
export function pushEvidence(s, item) { pushCapped(s.evidence, item, MAX_EVIDENCE); }
export function pushHypothesis(s, item) { pushCapped(s.hypotheses, item, MAX_HYPOTHESES); }
export function pushAudit(s, item) { pushCapped(s.audit, item, MAX_AUDIT); }
export function pushRecentEvent(s, text) {
    s.recentEvents.push({ text, ts: Date.now() });
    if (s.recentEvents.length > MAX_RECENT_EVENTS)
        s.recentEvents.shift();
}
export function updateSession(s, toolName, args, result) {
    s.calls.push({
        tool: toolName,
        ts: Date.now(),
