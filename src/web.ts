...
export function buildStats(s: SessionState): any {
  return {
    phase: s.phase,
    ...
    recent_calls: s.calls.slice(-10).reverse().map((c: any) => ({ tool: c.tool, ok: c.ok })),
    recent_events: s.recentEvents.slice(-5).reverse(),
  }
}

export function renderStatusHtml(s: SessionState): string {
  const stats = buildStats(s)
  const lockList = stats.locked_addresses.map((a: string) => `<li><code>${a}</code></li>`).join('') || '<li>none</li>'
  return `<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8" />
...
<div class="card"><h2>Recent calls</h2><ul>${stats.recent_calls.map((c: any) => `<li>${c.ok ? '✅' : '❌'} ${c.tool}</li>`).join('') || '<li>none</li>'}</ul></div>
...
export function injectStatusPanel(html: string): string {
