...
export const memoryReadDefs: ToolDef[] = [
  {
    name: 'ce_read_memory',
...
export const memoryWriteDefs: ToolDef[] = [
  {
...
export const memoryManyDefs: ToolDef[] = [
  {
    name: 'ce_read_many',
...
export const memoryUnifiedDefs: ToolDef[] = [
  {
    name: 'ce_memory_read',
    ...
    async execute(args: any, client: any) {
      ...
      if (mode === 'integer') return client.sendCommand('read_integer', { address: args.address, type: args.type || 'dword' })
      if (mode === 'memory') return client.sendCommand('read_memory', { address: args.address, size: Math.min(Number(args.size) || 256, 4096) })
      if (mode === 'string') return client.sendCommand('read_string', { address: args.address, max_length: Math.min(Number(args.max_length) || 256, 4096), encoding: args.encoding || 'utf8' })
  ...
  },
  {
    ...
    async execute(args: any, client: any) {
      ...
      if (mode === 'integer') {
        const address = String(args.address || '').trim()
        const type = args.type || 'dword'
        if (!address || !Number.isFinite(Number(args.value))) return { success: false, error: 'address and value are required', error_class: 'INVALID_ARGS' }
        const beforeRes = await client.sendCommand('read_integer', { address, type })
        const before = beforeRes && beforeRes.success !== false ? beforeRes.value : null
        const res = await client.sendCommand('write_integer', { address, value: Number(args.value), type })
